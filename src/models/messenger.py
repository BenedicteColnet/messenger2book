...
class MessengerModel(BaseChatAppModel):
    ...
    def _pre_process(self, raw_data):
        ...
        for message in raw_data["messages"][0:100000]:  # only 100 messages to test
            ...
            new_text = "".join(f"\emoji[{EMOJI_TYPE}]{{{ord(c):X}}}" if c in emoji.UNICODE_EMOJI else c for c in text) 
            ...
            #         new_text = new_text + \
            #             "\emoji[{EMOJI_TYPE}]{"+f"{hex(ord(c))[2:]}"+"}"
            ...
            if message.get('reactions') is not None:
            	...
            	new_react = "".join(f"\emoji[{EMOJI_TYPE}]{{{ord(c):X}}}" if c in emoji.UNICODE_EMOJI else c for c in react)
